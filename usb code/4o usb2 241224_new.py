...
def find_and_open_device():
    ...
    # Set timeouts (5s read, 5s write) - increased from 2s
    result = slabuart_dll.HidUart_SetTimeouts(device_handle, 5000, 5000)
...
def read_response(device_handle, max_loops=20):
    ...
    for _ in range(max_loops):
        ...
        if result == HID_UART_SUCCESS and bytes_read.value > 0:
            ...
            # No data or read timed out, longer pause
...
def main():
    ...
    try:
        ...
        time.sleep(2.0)  # Added 2s delay after connection

        # Attempt *IDN? first (removed *CLS)
        send_command(device, "*IDN?")
        time.sleep(0.5)  # Increased delay after command
        idn_response = read_response(device)
        if idn_response:
            print(f"[IDN?] => {idn_response}")
        else:
            print("[IDN?] => No response")

        # Example: read 3-channel voltages with proper delay
        time.sleep(0.5)
        send_command(device, "READ? VOLTS:CH1, VOLTS:CH2, VOLTS:CH3")
        time.sleep(0.5)  # Increased delay before reading
