...
def read_response(device, max_attempts=40):
    """
...
def stream_voltages(device):
    print("Streaming voltages from all channels...")
    try:
        while True:
            for chan in range(1, 4):  # Iterate over channels 1 to 3
                send_command(device, f"CHAN {chan}")
                time.sleep(0.1)  # Allow the channel to switch

                send_command(device, "READ? VOLTS:CHAN")
                response = read_response(device)
                if response:
                    print(f"Channel {chan} Voltage: {response} V")
                else:
                    print(f"Failed to read voltage from Channel {chan}")

                # Check for errors
                send_command(device, "*ERR?")
                error_response = read_response(device)
                if error_response and error_response != "0":
                    print(f"Device error detected: {error_response}")
...
def main():
    ...
    try:
        ...
        send_command(device, "MODE,0")  # Single VPA mode (m=0 for Single VPA)
        time.sleep(0.5)  # Wait for mode change to take effect

        # Confirm mode
        send_command(device, "MODE?")
        mode_response = read_response(device)
        print(f"Device mode: {mode_response}")

        # Configure channels for voltage readings
        for chan in range(1, 4):
            send_command(device, f"CHAN {chan}")
            send_command(device, f"VSCALE,{chan},1.0")  # Set voltage scaling
            time.sleep(0.5)  # Wait for configuration to settle

        send_command(device, "SAVECONFIG")
        time.sleep(1)  # Ensure configuration is saved
