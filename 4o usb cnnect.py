...
def find_and_open_device():
    ...
    result = slabuart_dll.HidUart_SetTimeouts(device, 2000, 2000)
...
def stream_voltages(device):
    ...
    try:
        while True:
            send_command(device, "READ? VOLTS:CH1")
            ch1_voltage = read_response(device)

            send_command(device, "READ? VOLTS:CH2")
            ch2_voltage = read_response(device)

            send_command(device, "READ? VOLTS:CH3")
            ch3_voltage = read_response(device)

            print(f"Channel 1: {ch1_voltage} V, Channel 2: {ch2_voltage} V, Channel 3: {ch3_voltage} V")
...
def main():
    ...
    try:
        ...
        send_command(device, "MODE,1")  # Multi-VPA mode

        send_command(device, "VSCALE,1,1.0")  # Configure channel 1
        send_command(device, "VSCALE,2,1.0")  # Configure channel 2
        send_command(device, "VSCALE,3,1.0")  # Configure channel 3
