...
def setup_dc_mode(ser):
    ...
    # Basic DC mode setup
    dc_setup = (":VOLT:MODE,DC;"
               ":VOLT:ALC,ON;:CURR:LIM,10;"
               ":VOLT:LIM:MIN,0;:VOLT:LIM:MAX,120;"
               ":OUTP,ON")
...
def set_dc_voltage(ser, voltage):
    ...
    try:
        if not 0 <= voltage <= 120:
            print(f"Warning: Voltage {voltage}V outside valid range (0-120V)")
        ...
        # Set voltage using exact command format from 3150Afx
        send_command(ser, f":VOLT:DC,{voltage}")
        time.sleep(1)
...
def main():
    ...
    try:
        ...
        if setup_dc_mode(ser):
            test_voltages = [10, 30, 60, 90, 120]  # Test points up to 120V
